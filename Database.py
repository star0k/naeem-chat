...
class Database:
    ...
    def fetch_chat_partners(self, username):
        ...
        messages_as_sender = (
            self.session.query(Message.recipient).filter_by(sender=username).all()
        )
        messages_as_recipient = (
            self.session.query(Message.sender).filter_by(recipient=username).all()
        )
    ...
    def register_user(self, username, hashed_password, email):
        new_user = User(
            username=username, password=hashed_password, email=email, isverified=False
        )
    ...
    def change_password(
        self, username, new_password, old_password=None, verification_code=None
    ):
        user = self.session.query(User).filter_by(username=username).one_or_none()
        if not user:
            return False
    ...
    def insert_user(
        self, username, hashed_password, email, fullname, verification_code
    ):
    ...
    def update_user_info(self, username, bio, native_language, interest_language):
        """
        Update the user's bio, native language, and interest language in the database.
        """
        try:
            user = self.session.query(User).filter_by(username=username).one_or_none()
