...
async def connect(sid, environ):
    global get
    username = next(
        (user for user, details in get.online_users.items() if details["sid"] == sid),
        None,
    )
    print(
        f"Connection Established with: {sid} user : {username if username else 'not auth yet'}"
    )
...
async def disconnect(sid):
    ...
    print(
        f"Connection closed with: {sid} user : {username if username else 'not auth yet'}"
    )
    if user_to_remove:
...
async def signin(sid, data):
    ...
    try:
        ...
        if db.authenticate_user(
            username, password
        ):  # Assuming `authenticate_user` checks the hashed password
            print("pass2")
            ...
            if user_data_result["profile_image"]:
                user_data_result[
                    "profile_image"
                ] = f"http://{get.HOST}:{get.PORT}/{user_data_result['profile_image']}"

            else:
                user_data_result["profile_image"] = ""
            await socket_service.emit(
                "signin-response",
                {"retcode": 0, "token": token, "data": user_data_result},
                to=sid,
            )
            ...
            await socket_service.emit(
                "signin-response",
                {"retcode": 1, "message": "Invalid credentials."},
                to=sid,
            )
        ...
        await socket_service.emit(
            "signin-response",
            {"retcode": 999, "message": "Unknown error occurred."},
            to=sid,
        )
    print(f"online users : {list(get.online_users.keys())}")
        ...
        if get.is_authenticated(sid, token, username):
            ...
            for partner in chat_partners:
                ...
                if partner_data["profile_image"]:
                    partner_data[
                        "profile_image"
                    ] = f"http://{get.HOST}:{get.PORT}/{partner_data['profile_image']}"
            ...
            await socket_service.emit(
                "chats-response", {"retcode": 0, "data": chat_data_list}, to=sid
            )
            ...
            await socket_service.emit(
                "chats-response",
                {"retcode": 3, "message": "Authentication failed."},
                to=sid,
            )
    ...
    try:
        ...
        if get.is_authenticated(sid, token, username):
            if partner in get.online_users:
                await socket_service.emit(
                    "online-response", {"retcode": 0, "messages": "online"}, to=sid
                )
        else:
            await socket_service.emit(
                "online-response", {"retcode": 1, "messages": "offline"}, to=sid
            )

    except Exception as e:
        print(f"Error: {e}")
        await socket_service.emit(
            "chats-response", {"retcode": 999, "message": e}, to=sid
        )
...
async def fetch_messages(sid, data):
    ...
    try:
        ...
        if get.is_authenticated(sid, token, username):
            ...
            await socket_service.emit(
                "messages", {"retcode": 0, "data": messages}, to=sid
            )
            print("sent data")
            ...
            for message in messages:
                ...
                if message["recipient"] == username and message["isread"] == 0:
                    ...
                    try:
                        ...
                        if (
                            sender not in senders_notified
                            and sender in get.online_users
                        ):
...
async def read_conversation(sid, data):
    try:
        ...
        if get.is_authenticated(sid, token, username):
            ...
            await socket_service.emit(
                "chat-read", {"retcode": 3, "message": "Authentication failed."}, to=sid
            )
    except:
        await socket_service.emit(
            "chat-read", {"retcode": 999, "message": "Unknown error occurred."}, to=sid
        )
        ...
        if get.is_authenticated(sid, token, username):
            if not db.username_exists(chat_partner):
                await socket_service.emit(
                    ...
                    {"retcode": 4, "message": "User Not Found."},
                    to=sid,
                )
                return
            ...
            if chat_partner in get.online_users:
                ...
                acknowledged = False
                try:
                    acknowledged = await socket_service.call(
                        "live",
                       {
                            "action": "new_message",
                            "data": {
                        ...
                        to=chat_partner_sid,
                        timeout=2,
                    )  # Await an acknowledgment for up to 2 seconds
                except Exception as e:
                    print(e)
...
async def signup(sid, data):
    try:
        ...
        if not get.is_valid_email(email):
            ...
            await socket_service.emit(
                "signup-response",
                {"retcode": 1, "message": "Invalid email format."},
                to=sid,
            )
            return
        ...
        if db.email_exists(email):
            ...
            await socket_service.emit(
                "signup-response",
                {"retcode": 2, "message": "Email Already Registered."},
                to=sid,
            )
            return
        ...
        if db.username_exists(username):
            ...
            await socket_service.emit(
                "signup-response", {"retcode": 3, "message": "Username exists."}, to=sid
            )
            return
        ...
        await socket_service.emit(
            "signup-response",
            {"retcode": 999, "message": f"Unknown error occurred. {e}"},
            to=sid,
        )
...
async def fetch_messages_status(sid, data):
    try:
        ...
        if get.is_authenticated(sid, token, username):
            ...
            status_updates = [
                {"message_id": message["id"], "isread": message["isread"]}
                for message in messages
            ]
        ...
        if get.is_authenticated(sid, token, username):
            if db.verify_email(username, verification_code):
                await socket_service.emit(
                    ...
                    {"retcode": 0, "message": "user verified"},
                    to=sid,
                )
            else:
                await socket_service.emit(
                    "verification-response",
                    {"retcode": 1, "message": "Wrong code"},
                    to=sid,
                )
    ...
    try:
        ...
        if get.is_authenticated(sid, token, username):
            if db.change_password(
                username, newpassword, oldpassword, verification_code
            ):
                await socket_service.emit(
                    "change-password-response",
                    {"retcode": 0, "message": "passwor dchanged"},
                    to=sid,
                )
            else:
                await socket_service.emit(
                    "verification-response", {"retcode": 1, "message": "Wrong "}, to=sid
                )
        ...
        if db.set_verification_code(username, verification_code):
            ...
            if email:
                ...
                await socket_service.emit(
                    ...
                    {
                        "retcode": 0,
                        "message": "Code sent. Please check email for verification code.",
                    },
                    to=sid,
                )
    except:
...
@socket_service.on("request-call")
async def request_call(sid, data):
    try:
        ...
        if not get.is_authenticated(sid, token, sender):
            await socket_service.emit(
                "request-call-respnose",
                {"retcode": 3, "message": "Authentication failed."},
                to=sid,
            )
            return
        ...
        if recipient in get.online_users:
            recipient_sid = get.online_users[recipient]["sid"]
            # Emit a call request event to the recipient
            await socket_service.emit(
                "live", [{"action": "incoming-call", "sender": sender,"isVideo": isVideo}], to=recipient_sid
            )
            await socket_service.emit(
                "request-call-respnose",
                {"retcode": 0, "message": "Call request sent successfully."},
                to=sid,
            )
        else:
            await socket_service.emit(
                "request-call-respnose",
                {"retcode": 1, "message": "Recipient is not online."},
                to=sid,
            )
    except Exception as e:
        print(f"Error: {e}")
...
async def accept_call(sid, data):
    try:
        ...
        if not get.is_authenticated(sid, token, recipient):
            await socket_service.emit(
        ...
        if sender in get.online_users:
            sender_sid = get.online_users[sender]["sid"]
            # Emit a call response event to the sender indicating that the call is accepted
            await socket_service.emit(
                "live",
                [{"action": "call-accepted", "recipient": recipient}],
                to=sender_sid,
            )
            await socket_service.emit(
                "accept-call-response",
                {"retcode": 0, "message": "Call accepted."},
                to=sid,
            )
        else:
            await socket_service.emit(
                "accept-call-response",
                {"retcode": 1, "message": "Sender is not online."},
                to=sid,
            )
    except Exception as e:
        print(f"Error: {e}")
...
async def reject_call(sid, data):
    try:
        ...
        if sender in get.online_users:
            sender_sid = get.online_users[sender]["sid"]
            # Emit a call response event to the sender indicating that the call is rejected
            await socket_service.emit(
                "live",
                [{
            ...
            await socket_service.emit(
                "reject-call-response",
                {"retcode": 0, "message": "Call rejected."},
                to=sid,
            )
        else:
            await socket_service.emit(
...
async def advance_search_users(sid, data):
    try:
        ...
        # Loop through the results and update profile_image URLs
        for result in results:
            if result["profile_image"]:
                result[
                    "profile_image"] = f"http://{get.HOST}:{get.PORT}/{result['profile_image']}"  # Use 'result' instead of 'results'
            else:
                result["profile_image"] = ""

        print(results)
        await socket_service.emit(
            "advanced-search-response", {"retcode": 0, "data": results}, to=sid
        )
    except Exception as e:
...
async def update_user_info(sid, data):
    try:
        username = data.get("username")
        token = data.get("token")
        bio = data.get("bio")
        ...
        if get.is_authenticated(sid, token, username):
            ...
            db.update_user_info(username, bio, native_language, interest_language)

            # Emit a response back to the user
            await socket_service.emit(
                "update-user-info-response",
                {"retcode": 0, "message": "User information updated successfully."},
                to=sid,
            )
...
async def upload_profile_image(sid, data):
    try:
        ...
        if not os.path.exists(directory):
            os.makedirs(
                directory
            )  # this line will create the directory if it doesn't exist
        ...
        if file_data and get.allowed_file(filename):
            ...
            image_path = os.path.join(
                "profile_images",
                f"{username}{random.randint(1000, 9999)}.{filename.rsplit('.', 1)[1].lower()}",
            )
            ...
            await socket_service.emit(
                "upload-image-response",
                {"retcode": 0, "message": "Image uploaded successfully."},
                to=sid,
            )
            ...
            await socket_service.emit(
                "upload-image-response",
                {"retcode": 1, "message": f"Invalid file format.{filename}"},
                to=sid,
            )
